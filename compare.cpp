...
bool co(int be,int en) {
	...
	while(nn<=en) {
		if(lo[nn]=='<') {
			...
			++now;
			if(now>10000000) {
				printf("超内存限制:我不懂你为什么需要这么大的内存……超过10,000,000字节!\n");
				aaa=false;
				printf("-----------------------------------\n");
				lk=lk+3+(q-1)/175;
			}
...
void imt() {
	...
	colset(15);
	system("cls");
	printf("你使用更少的内存，输出，代码和时间时，你将使用它\n");
	printf("你");
	colset(12);
	printf("确定");
	colset(15);
...
void hlp() {
	system("cls");
	printf("Brainf**k(河蟹)是一种极小化的计算机语言，它是由Urban Müller在1993年创建的。\n");
	printf(">指针右移一位指向下一个字节\n<指针左移一位指向上一个字节\n+当前指针指向的字节+1\n-当前指针指向的字节-1\n");
	printf(".以字符的形式输出当前指针指向的字节(ASCII码)\n,以ASCII的形式读入到当前指针指向的字节(ASCII码)\n");
	printf("[开始一个循环\n]如果指针指向0就跳出循环继续执行,否则跳到循环开始\n");
	printf("+-----+\n|关闭 |\n|    C|\n+-----+");
	while(1) {
		Sleep(50);
		got();
...
void imf() {
	colset(15);
	system("cls");
	printf("这是一个Brainf**k(河蟹)编译系统\n");
	printf("开发者信息                   |修改者信息                   |汉化者信息\n");
	printf("名字:\n");
	printf("++++++++[->++++++++<]>+++    |+++++++++[->+++++++++<]>+    |>++++++++++[<++++++++++++>\n");
	printf("++<]>+++.<.<++++++[->++++    |++++++++.++++++++.<++++[-    |-]<.------.+++++++.\n");
	printf("+++[->+++<]>+.<              |>++++<]>+++++.<              |\n");
...
int main(int argc,char* argv[]) {
	...
	while(1) {
		system("cls");
		colset(15);
		printf("编写By cjt->https://www.luogu.org/blog/tiger2005/bf-bian-yi-qi\n");
		printf("汉化By xry->https://www.luogu.org/blog/xryjr233/bf-bian-yi-qi-han-hua-ban-bian-xie-by-cjt-han-hua-by-xry\n");
		printf("|+-------+  +-----+  +----+|");
		printf("|+----------+  +----+|");
		if(!r)  colset(8);
		printf("|+---+  +----------+  +-----+|\n");
